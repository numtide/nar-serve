...
`
	...
	strNarinfoSampleCachix = `
StorePath: /nix/store/8nb4qdm1n2mpfcfr3hdaxw54fjdn4hqz-treefmt-docs-4b33ba3
URL: nar/b136fa7b36b966d63a93f983ee03070e44bffe9ba9005cda59835e2a0f0f64b9.nar.zst
Compression: zstd
FileHash: sha256:b136fa7b36b966d63a93f983ee03070e44bffe9ba9005cda59835e2a0f0f64b9
FileSize: 873969
NarHash: sha256:1d3pp407iawzv79w453x5ff5fs0cscwzxm7572q85nijc56faxr8
NarSize: 1794360
References: 
Deriver: 7nzyn0l9402ya02g6sac073c3733k0p7-treefmt-docs-4b33ba3.drv
Sig: numtide.cachix.org-1:YYcsiDnC0WR2utXGy1G6PqjDPH7TsvMrpaK4QJV1MHLks4N5XPA+Na0yzfOqBxqn9BB8NsTAqSu2B08SiIQmDA==
`

	_NarHash = nixhash.MustNewHashWithEncoding(nixhash.SHA256, []uint8{
	...
	narinfoSample = &narinfo.NarInfo{
		...
		URL:         "nar/1094wph9z4nwlgvsd53abfz8i117ykiv5dwnq9nnhz846s7xqd7d.nar.xz",
		Compression: "xz",
		FileHash: nixhash.MustNewHashWithEncoding(nixhash.SHA256, []byte{
	...
	narinfoSampleCachix = &narinfo.NarInfo{
		StorePath:   "/nix/store/8nb4qdm1n2mpfcfr3hdaxw54fjdn4hqz-treefmt-docs-4b33ba3",
		URL:         "nar/b136fa7b36b966d63a93f983ee03070e44bffe9ba9005cda59835e2a0f0f64b9.nar.zst",
		Compression: "zstd",
		NarHash: nixhash.MustNewHashWithEncoding(nixhash.SHA256, []byte{
			0x28, 0x77, 0xe5, 0x4c, 0x61, 0x32, 0xda, 0x82, 0xb0, 0x38, 0xe5, 0xd4, 0xfe, 0x39, 0xd3, 0x0c,
			0x68, 0x57, 0x9c, 0x2b, 0x7d, 0x14, 0xc2, 0xd3, 0xd9, 0x9f, 0xab, 0x78, 0x00, 0xb9, 0x77, 0xb4,
		}, nixhash.NixBase32, true),
		NarSize: 1794360,
		FileHash: nixhash.MustNewHashWithEncoding(nixhash.SHA256, []byte{
			0xb1, 0x36, 0xfa, 0x7b, 0x36, 0xb9, 0x66, 0xd6, 0x3a, 0x93, 0xf9, 0x83, 0xee, 0x03, 0x07, 0x0e,
			0x44, 0xbf, 0xfe, 0x9b, 0xa9, 0x00, 0x5c, 0xda, 0x59, 0x83, 0x5e, 0x2a, 0x0f, 0x0f, 0x64, 0xb9,
		}, nixhash.Base16, true),
		FileSize: 873969,
		Deriver:  "7nzyn0l9402ya02g6sac073c3733k0p7-treefmt-docs-4b33ba3.drv",
		Signatures: []*narinfo.Signature{
			{
				KeyName: "numtide.cachix.org-1",
				Digest: []byte{
					0x61, 0x87, 0x2c, 0x88, 0x39, 0xc2, 0xd1, 0x64, 0x76, 0xba, 0xd5, 0xc6, 0xcb, 0x51, 0xba, 0x3e,
					0xa8, 0xc3, 0x3c, 0x7e, 0xd3, 0xb2, 0xf3, 0x2b, 0xa5, 0xa2, 0xb8, 0x40, 0x95, 0x75, 0x30, 0x72,
					0xe4, 0xb3, 0x83, 0x79, 0x5c, 0xf0, 0x3e, 0x35, 0xad, 0x32, 0xcd, 0xf3, 0xaa, 0x07, 0x1a, 0xa7,
					0xf4, 0x10, 0x7c, 0x36, 0xc4, 0xc0, 0xa9, 0x2b, 0xb6, 0x07, 0x4f, 0x12, 0x88, 0x84, 0x26, 0x0c,
				},
			},
		},
	}
...
func TestNarInfoCachix(t *testing.T) {
	ni, err := narinfo.Parse(strings.NewReader(strNarinfoSampleCachix))
	assert.NoError(t, err)

	// Test the parsing happy path
	assert.Equal(t, narinfoSampleCachix, ni)
	assert.NoError(t, ni.Check())

	// Test to string
	assert.Equal(t, strNarinfoSampleCachix, "\n"+ni.String())
}
